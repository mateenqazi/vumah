...
function PasswordVerification(props) {
  ...
  const [VerifyNewPassword, { loading, data, error }] = useMutation(PASSWORD_VERIFICATION, {
                ...
                <MotionContainer initial="initial" open>
                  <Box sx={{ maxWidth: 680, margin: 'auto', textAlign: 'center' }}>
                    ...
                    <motion.div variants={varFadeInDown}>
                      <Paper sx={{ p: 2 }}>
                        <Typography component="snap" variant="body1" sx={{ color: 'text.primary' }}>
                          Your password was not updated due to some errors, you can try again or contact our support
                          team
                        </Typography>
                      </Paper>
                    </motion.div>
